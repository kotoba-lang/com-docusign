(ns docusign.main
  "Kotodama WASM entrypoint for the Docusign clean-room actor (L4) — Clojure port.
  ...
  facts (`docusign.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "docusign")
(def tier "L4")
...
(def entity-specs
  [{:entity "Envelope"          :plural "envelopes"          :id-prefix "docusign_env"
    :fields [:emailSubject :status :sentDateTime :completedDateTime :envelopeId] :required [:emailSubject :status]
    :coerce {}                       :refs {}}
   {:entity "Signer"            :plural "signers"            :id-prefix "docusign_sig"
    :fields [:email :firstName :lastName :status :signedDateTime]           :required [:email :firstName]
    :coerce {}                       :refs {}}
   {:entity "Document"          :plural "documents"          :id-prefix "docusign_doc"
    :fields [:documentId :name :order]                        :required [:name :order]
    :coerce {:order :int}            :refs {}}
   {:entity "SignHereTab"       :plural "signheretabs"       :id-prefix "docusign_tab"
    :fields [:tabId :pageNumber :xPosition :yPosition]       :required [:pageNumber :xPosition]
    :coerce {}                       :refs {}}
   {:entity "EnvelopeTemplate"  :plural "envelopetemplates"  :id-prefix "docusign_tpl"
    :fields [:templateId :name :status :description]         :required [:name :status]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"docusign.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "docusign-compat" :tier tier :entities entities} 200])
...
(defn start! [] :docusign-compat/ready)
